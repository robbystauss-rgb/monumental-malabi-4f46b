...
import hatCatalogModule from './hat-options.mjs';
...
const catalog = {
  'Richardson 112': {
    ...
    options: ['Royal','Black','Red','Quarry','Khaki/Coffee','Blue Teal/Birch/Navy','White/Red','White/Royal']
  },
  'Richardson 168': {
    label: 'Richardson 168 Seven-Panel',
  ...
  'Richardson 112PFP': {
    label: 'Richardson 112PFP Printed Five Panel',
    options: ['Realtree Edge/Brown','Green Camo/Black','Digital Camo/Light Green','Green Camo/White','Desert Camo/Brown','Realtree Edge/Neon Orange','Admiral Duck Camo/Black','Realtree Edge/Neon Pink','Kryptek Highlander/Buck','Bark Duck Camo/Brown','Kryptek Inferno/Blaze','Realtree Edge/Neon Yellow','Blaze Duck Camo/Blaze','Realtree Escape/Black','Kryptek Neptune/White','Kryptek Typhon/Black','Realtree Fishing Light Blue/White','Kryptek Typhon/Blaze','Realtree Max-1 XT/Brown','Blizzard Duck Camo/White','Harvest Duck Camo/Light Tan','Realtree Max-7/Buck','Marsh Duck Camo/Loden','Sable Duck Camo/Black','Realtree Original/Black','Realtree Timber/Black','Saltwater Duck Camo/Charcoal','Mossy Oak Bottomland/Loden','Mossy Oak Country DNA/Black','Mossy Oak Elements Bonefish/Light Grey','Mossy Oak Habitat/Brown','Sienna Duck Camo/Loden']
  }
};
...
export default async (request) => {
  try {
    ...
    if (!EXPECTED_TITLE.test(listing.title || '')) {
      return json({ error: 'Safety stop: listing title does not look like a custom hat listing.', listing_id: listingId, title: listing.title }, 409);
    }
    ...
    const products = [];
    for (const [model, data] of Object.entries(catalog)) {
      for (const color of data.options) {
        products.push({
          sku: '',
          offerings: [{ ...offering }],
          property_values: [
    ...
    if (body.dry_run !== false) {
      return json({
        ...
        models: Object.fromEntries(Object.entries(catalog).map(([k,v]) => [k, v.options.length])),
